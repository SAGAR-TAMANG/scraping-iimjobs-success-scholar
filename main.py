...
print("ALL THE DATA HAS BEEN FETCHED - NOW INTEGRATION HAS BEGUN PLEASE WAIT")

dff101 = pd.DataFrame(columns=['Job Title', 'Experience Reqd', 'City', 'Date Posted', 'Site', 'URL'])

dff101 = pd.concat([dff101, dff1])
dff101 = pd.concat([dff101, dff2])
dff101 = pd.concat([dff101, dff3])
dff101 = pd.concat([dff101, dff4])
dff101 = pd.concat([dff101, dff5])
dff101 = pd.concat([dff101, dff6])
dff101 = pd.concat([dff101, dff7])
dff101 = pd.concat([dff101, dff8])
